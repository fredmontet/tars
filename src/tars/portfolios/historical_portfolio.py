...
class HistoricalPortfolio(AbstractPortfolio):
    """
    ...
    def __init__(self, market: HistoricalMarket, account: dict):
    ...
    def get_trade_balance(self, timestamp: Timestamp, asset: str = 'ZUSD') -> DataFrame:
        ...
        if currencies:
            ...
            for c in currencies:
                pair = c + quote
                tf = self.market.get_ticker_information(timestamp, pair)
                close = float(tf['c'])
